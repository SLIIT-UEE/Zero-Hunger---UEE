...
const {
  ...
  updateFoodWastageStatus,
  getFoodWastageByHotelOwner,
  addVolunteer,
  volunteerLogin,
  deleteFoodWastage,
...
router.get("/organization/profile", authMiddleware, (req, res) => {
  getOrganizationProfile(req, res);
  res.status(200).json(req.user);
});
router.put("/organization/profile", authMiddleware, (req, res) => {
  updateOrganizationProfile(req, res); // This should handle the response
});
router.get("/foodlist", authMiddleware, getFoodDetails);
router.put("/food/:id", authMiddleware, updateFoodNeedStatus);
