...
const foodneeds = require("../models/foodneeds");
...
const getFoodDetails = async (req, res) => {
  try {
    const foodList = await foodneeds
      .find()
      .populate("organizationId", "orgName location") // Populate organization details
      .select("organizationId foodItem people requiredDate status"); // Select necessary fields

    res.status(200).json({ success: true, data: foodList });
  } catch (error) {
    console.error("Error retrieving food need details:", error);
    res.status(500).json({ success: false, message: "Server error" });
  }
};

const updateFoodNeedStatus = async (req, res) => {
  const { id } = req.params; // Extract the ID from the URL parameters
  const { status } = req.body; // Extract the new status from the request body

  try {
    // Find the food need item by ID and update the status
    const updatedFoodNeed = await foodneeds.findByIdAndUpdate(
      id,
      { status: status }, // Update the status field
      { new: true, runValidators: true } // Options: return the new document and run validators
    );

    // Check if the food need item was found and updated
    if (!updatedFoodNeed) {
      return res.status(404).json({ message: "Food need item not found." });
    }

    // Send the updated food need item back in the response
    res
      .status(200)
      .json({ message: "Status updated successfully!", data: updatedFoodNeed });
  } catch (error) {
    console.error("Error updating food need status:", error);
